...
def basic_setup():
    ...
    print(
        f"Created group: ID={group['id']}, Name={group['name']}, Admin ID={group['adminId']}"
    )

    # Add user2 to the group using the helper function
    print("\nAdding user2 to the group...")
    accepted_group = add_user_to_group(
        group["id"], user2["id"], user1_token, user2_token
    )
